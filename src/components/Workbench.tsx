...
export function WorkbenchNav({page,onPage,project,projects,onProject}: {page:Page;onPage:(page:Page)=>void;project:Project|null;projects:Project[];onProject:(p:Project)=>void}) {
 return <header className="workbench-nav">
  <div className="brand-context">
   <button className="wordmark" onClick={()=>onPage('welcome')}>StoryGuard</button>
   <label className="project-switch"><span className="sr-only">현재 작품</span><select value={project?.id ?? ''} onChange={e=>{const p=projects.find(p=>p.id===Number(e.target.value));if(p)onProject(p);}}><option value="" disabled>작품 선택</option>{projects.map(p=><option key={p.id} value={p.id}>{p.title}</option>)}</select></label>
  </div>
  <nav className="project-navigation" aria-label="작품 메뉴">{MENU.map(([key,Icon])=><button key={key} aria-label={PAGES[key][0]} title={PAGES[key][0]} aria-current={page===key?'page':undefined} onClick={()=>onPage(key)}><Icon size={17} aria-hidden="true"/><span>{PAGES[key][0]}</span></button>)}</nav>
  <div className="appearance-navigation"><ThemePicker/>
   <nav className="nav-bottom" aria-label="앱 메뉴"><button aria-label="내 작품" title="내 작품" aria-current={page==='projects'?'page':undefined} onClick={()=>onPage('projects')}><BookOpen size={20} aria-hidden="true"/></button><button aria-label="앱 설정" title="앱 설정" aria-current={page==='settings'||page==='setup'?'page':undefined} onClick={()=>onPage('settings')}><Settings size={20} aria-hidden="true"/></button></nav>
  </div>
 </header>;
}
export function ProjectsPage({projects,onOpen,onCreate}: {projects:Project[];onOpen:(p:Project)=>void;onCreate:()=>void}) {
 return <div className="page-content"><div className="section-heading"><h2>최근 작품 <span className="count">{projects.length}</span></h2><button className="primary" onClick={onCreate}>+ 새 작품 만들기</button></div><div className="project-grid">{projects.map(p=><button className="project-card" key={p.id} onClick={()=>onOpen(p)}><BookOpen size={32}/><h2>{p.title}</h2><p>{p.document_count ?? 0}편 · {p.pending_document_count ? `재분석 필요 ${p.pending_document_count}편` : p.last_analyzed_at ? `최근 분석 ${formatProjectDate(p.last_analyzed_at)}` : '아직 분석하지 않음'}</p>{(p.open_issue_count ?? 0) > 0 && <span className="project-issue-count">확인 대기 {p.open_issue_count}개</span>}<span>작품 열기 <ArrowRight size={16}/></span></button>)}</div>{!projects.length&&<div className="blank-state">첫 작품을 만들어 원고를 가져오세요.</div>}</div>;
}
...
export function ReviewPage({graph,evidence,documents,onStatus,onGraph,onOpenDocument,onAnalysis,history}: {history:import("../lib/types").ReviewHistory[];graph:GraphPayload;evidence:Record<number,EvidenceChunk[]>;documents:StoryDocument[];onStatus:(id:number,s:IssueStatus)=>Promise<boolean> | void;onGraph:(relationId?:number)=>void;onOpenDocument:(id:number,quote?:string)=>void;onAnalysis:()=>void}) {
...
export function GraphDetails({graph,entity,relationId,onRelation,onReview,onOpenDocument,documents=[]}: {graph:GraphPayload;entity:EntityNode|null;relationId:number|null;onRelation:(id:number)=>void;onReview:()=>void;onOpenDocument:(id:number,quote?:string)=>void;documents?:StoryDocument[]}) {
 ...
 return <aside className="surface graph-details" tabIndex={0} aria-label="관계 설명과 원문 근거, 스크롤 가능">
  <div className="graph-evidence-heading">
   <div><span className="eyebrow">{edge ? edgeStatus : entity ? '대상의 기록' : '관계 지도'}</span><h2>{entity ? entity.name : edge ? '두 대상의 이야기를 원문에서 확인하세요' : '연결된 이야기를 살펴보세요'}</h2></div>
   {edge && <button className="text-action" onClick={()=>onRelation(-1)}>← 관계 목록</button>}
  </div>
  {entity && <><span className="badge">추출 후보 · 연결 {edges.length}개</span>{entity.summary && <p>{entity.summary}</p>}<p className="muted">{entity.document_count}개 회차에서 언급됨</p>
   {!edges.length && <div className="soft-card"><h3>연결이 확인되지 않은 후보</h3><p>현재 표시 범위에 관계가 없습니다. 잘못 추출된 이름이나 분류일 수도 있으므로 원문을 확인하세요.</p>{entity.document_ids.map(id=><button className="text-action" key={id} onClick={()=>onOpenDocument(id)}>{documents.find(d=>d.id===id)?.title ?? '원문'} 열기</button>)}</div>}
  </>}
  {!edge && edges.length > 0 && <><p className="muted">{entity ? '확인할 관계를 선택하세요.' : '선을 선택하면 관계의 의미와 원문을 아래에서 확인할 수 있습니다.'} 같은 대상·같은 의미의 후보는 묶어서 표시합니다.</p>
   <div className="relationship-list">{groupedEdgesForDisplay.map(({representative:r,count,evidenceCount,claimCount,origins})=><button key={r.id} onClick={()=>onRelation(r.id)}>
    <span>{entityName(r.source_entity_id)} → {entityName(r.target_entity_id)}</span><strong>{r.type==='관계' ? '유형 미분류' : (r.display_label || r.type)}{count>1 && <em className="relation-count">후보 {count}개</em>}</strong>
    <span className="relationship-summary">{r.claims?.[0]?.explanation || '관계 설명을 원문 근거에서 확인하세요.'}</span><small>{origins.size>1 ? '로컬·GPT' : (r.origin==='gpt' ? 'GPT' : '로컬')} 추출 · 근거 {evidenceCount}개{claimCount ? ' · 주장 '+claimCount+'개' : ''}</small>
   </button>)}</div>
  </>}
  {edge && <>
   <RelationStory edge={edge} graph={graph} documents={documents} onOpenDocument={onOpenDocument}/>
   <details className="graph-context-details"><summary>관계 상세·회차별 관찰·전체 근거 구간</summary>
    <div className="soft-card"><span className="badge">{edge.origin==='gpt' ? 'GPT' : '로컬'} 추출 후보</span><span className={'relation-status '+(edgeStatus==='충돌 후보' ? 'danger' : edgeStatus==='근거 확인' ? 'good' : 'warn')}>{edgeStatus}</span><h3>{entityName(edge.source_entity_id)} → {entityName(edge.target_entity_id)}</h3><p className="relation-predicate">{edge.type==='관계' ? '유형 미분류' : edge.type}</p><span className="muted">원문 근거 {edge.evidence_chunk_ids.length}개 구간 · 같은 쌍 후보 {pairEdges.length}개</span></div>
    {pairEdges.length>1 && <div className="relation-timeline relation-candidates"><h3>같은 대상의 다른 관계 후보</h3><p className="muted">한 쌍에 여러 시점·모델의 주장이 있습니다. 각각의 근거를 비교해 작가가 판단하세요.</p>{pairEdges.map(candidate=><button className={candidate.id===edge.id ? 'selected' : ''} key={candidate.id} onClick={()=>onRelation(candidate.id)}><strong>{candidate.type==='관계' ? '유형 미분류' : (candidate.display_label || candidate.type)}</strong><span>{candidate.origin==='gpt' ? 'GPT' : '로컬'} · 근거 {candidate.evidence_chunk_ids.length}개{candidate.claims?.length ? ' · 주장 '+candidate.claims.length+'개' : ''}</span></button>)}</div>}
    {edgeTimeline.length>0 && <div className="relation-timeline"><h3>회차별 관찰</h3>{edgeTimeline.map(item=><div key={item.document_id+'-'+item.relation_type}><span>{item.chapter_index+1}화</span><strong>{item.relation_type}</strong>{item.status==='changed' && <em>변화</em>}{item.status==='gap' && <em>중간 공백</em>}{item.status==='explicit_break' && <em className="danger-text">명시적 단절</em>}</div>)}{timelineHasGap && <p className="muted timeline-note">중간 회차에 관계가 언급되지 않은 후보입니다. 단절 확정이 아니라 원문 확인 대상으로 표시합니다.</p>}</div>}
    <h3>전체 근거 구간</h3><RelationEvidence key={edge.id} relationId={edge.id} expanded onOpenDocument={onOpenDocument} documents={documents}/>
   </details>
   <div className="graph-evidence-actions"><p className="muted">시점이 다른 관계도 함께 표시됩니다. 원문을 확인해 판단해 주세요.</p><button className="primary" onClick={onReview}>검토 결과로 이동 <ArrowRight size={18}/></button></div>
  </>}
  {!edges.length && !entity && <div className="blank-state"><Network size={36}/><p>표시할 관계가 없습니다. 회차·유형 필터와 분석 상태를 확인하세요.</p></div>}
 </aside>;
}
